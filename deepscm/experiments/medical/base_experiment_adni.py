...
class BaseSEM(PyroModule):
    ...
    def infer_exogeneous(self, obs):
        ...
        for name, node in cond_trace.nodes.items():
            if 'z' in name or 'fn' not in node.keys(): #care of the z which was added to the dict in def infer():
                continue
...
class BaseCovariateExperiment(pl.LightningModule):
    ...
    def prepare_data(self):
        ...
        self.pyro_model.tau_flow_lognorm_loc = (self.adni_train.metrics['tau'].log().mean().to(self.torch_device).float())
        self.pyro_model.tau_flow_lognorm_scale = (self.adni_train.metrics['tau'].log().std().to(self.torch_device).float())
        ...
        self.pyro_model.education_flow_lognorm_loc = (self.adni_train.metrics['education'].log().mean().to(self.torch_device).float())
        self.pyro_model.education_flow_lognorm_scale = (self.adni_train.metrics['education'].log().std().to(self.torch_device).float())
    ...
    def val_dataloader(self):
        self.val_loader = DataLoader(self.adni_val, batch_size=self.test_batch_size, shuffle=True, **self._dataloader_params())
    ...
    def build_counterfactual(self, tag, obs, conditions, absolute=None):
        ...
        self.log_img_grid(tag, torch.cat(imgs, 0)) #maybe save and see what this does. 
    ...
    def sample_images(self):
        with torch.no_grad():
            ...
            obs_batch = {k: v[:8] for k, v in obs_batch.items()} # select the first eight.
